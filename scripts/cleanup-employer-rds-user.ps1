# cleanup-employer-rds-user.ps1
#
# Deletes one employer from RDS and the employer Cognito pool so you can retest
# employer account creation for that email address.
#
# Usage:
#   cd infra
#   npx cdk deploy JaleBastionStack
#   cd ..
#   .\scripts\cleanup-employer-rds-user.ps1
#   # Prompts for the employer email to delete.
#   # Or run non-interactively:
#   .\scripts\cleanup-employer-rds-user.ps1 -Email 'employer@example.com'
#   # If pool lookup by name is unavailable, pass it explicitly:
#   .\scripts\cleanup-employer-rds-user.ps1 -Email 'employer@example.com' -EmployerPoolId 'us-east-2_abc123'
#   cd infra
...
if ([string]::IsNullOrWhiteSpace($Email)) {
  $Email = Read-Host 'Enter employer email to delete from RDS and Cognito'
}

$Email = $Email.Trim()

if ($Email -notmatch '^[^@\s]+@[^@\s]+\.[^@\s]+$') {
  throw "Email must be a valid email address, for example employer@example.com."
}

$EmailSql = $Email.Replace("'", "''")

Write-Host ">> Using region: $Region"
Write-Host ">> Cleaning employer data for: $Email"

if ([string]::IsNullOrWhiteSpace($EmployerPoolId)) {
  Write-Host ">> Resolving employer Cognito pool ID..."
  $EmployerPoolId = (aws cognito-idp list-user-pools `
      --region $Region `
      --max-results 60 `
      --query "UserPools[?Name=='jale-employer-pool'].Id | [0]" `
      --output text).Trim()

  if ([string]::IsNullOrEmpty($EmployerPoolId) -or $EmployerPoolId -eq 'None') {
    throw "Could not find Cognito user pool named jale-employer-pool in $Region. Pass -EmployerPoolId explicitly."
  }
}
Write-Host "   employer-pool: $EmployerPoolId"
...
WHERE user_type = 'employer'
  AND (
    lower(email) = lower('$EmailSql')
    OR cognito_sub = '$EmailSql'
  );
...
BEGIN
  IF to_regclass('jobs') IS NOT NULL THEN
    ...
    SELECT id
    FROM jobs
    WHERE employer_id IN (SELECT id FROM cleanup_user_ids)
...
BEGIN
  IF to_regclass('document_upload_tokens') IS NOT NULL THEN
    ...
    WHERE job_id IN (SELECT id FROM cleanup_job_ids);
  ...
  IF to_regclass('employer_candidate_rankings') IS NOT NULL THEN
    DELETE FROM employer_candidate_rankings
    WHERE job_id IN (SELECT id FROM cleanup_job_ids);
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    INSERT INTO cleanup_counts(table_name, deleted_count)
    VALUES ('employer_candidate_rankings', deleted_count)
    ON CONFLICT (table_name) DO UPDATE
      SET deleted_count = cleanup_counts.deleted_count + EXCLUDED.deleted_count;
  END IF;
  ...
  IF to_regclass('jobs') IS NOT NULL THEN
    ...
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    INSERT INTO cleanup_counts(table_name, deleted_count)
    VALUES ('jobs', deleted_count)
    ON CONFLICT (table_name) DO UPDATE
      SET deleted_count = cleanup_counts.deleted_count + EXCLUDED.deleted_count;
  END IF;

  IF to_regclass('employer_profiles') IS NOT NULL THEN
    DELETE FROM employer_profiles
    WHERE user_id IN (SELECT id FROM cleanup_user_ids);
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    INSERT INTO cleanup_counts(table_name, deleted_count)
    VALUES ('employer_profiles', deleted_count)
    ON CONFLICT (table_name) DO UPDATE
      SET deleted_count = cleanup_counts.deleted_count + EXCLUDED.deleted_count;
  END IF;
...
WITH deleted_users AS (
  ...
  WHERE id IN (SELECT id FROM cleanup_user_ids)
     OR (user_type = 'employer' AND lower(email) = lower('$EmailSql'))
  RETURNING 1
)
INSERT INTO cleanup_counts(table_name, deleted_count)
...
TMP="/tmp/jale-cleanup-employer-user.sql"
...
echo ">> Employer RDS user cleanup completed for $Email."
"@
...
try {
  ...
  $cmdId = (aws ssm send-command `
      ...
      --comment "Jale employer RDS cleanup for $Email" `
...
Write-Host ">> Deleting Cognito employer user from pool $EmployerPoolId..."
$cognitoDeleteOutput = & aws cognito-idp admin-delete-user `
  --region $Region `
  --user-pool-id $EmployerPoolId `
  --username $Email 2>&1

if ($LASTEXITCODE -eq 0) {
  Write-Host ">> Cognito employer user deleted for $Email."
}
else {
  $message = ($cognitoDeleteOutput | Out-String).Trim()
  if ($message -match 'UserNotFoundException') {
    Write-Host ">> Cognito employer user was already absent for $Email."
  }
  else {
    throw "Cognito employer delete failed for $Email`: $message"
  }
}
