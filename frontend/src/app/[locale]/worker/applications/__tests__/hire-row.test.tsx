...
import type { Application, ApplicationsPage, ApplicationHire } from '@/lib/api/worker';
...
describe('worker applications -- the hired row', () => {
  ...
  it('leaves the compact banner alone for a pre-095 hire with no trade or company', () => {
    ...
    seed = pageOf([application({ hire: hire({ trade: undefined, company: undefined }) })]);
    renderIntl(<WorkerApplicationsPage />);
  ...
  it('leaves out the facts the hire does not carry', () => {
    seed = pageOf([application({ hire: hire({ start_date: null, location: null, pay: null }) })]);
    renderIntl(<WorkerApplicationsPage />);
  ...
  it('shows no banner on a hire that was already acknowledged', () => {
    seed = pageOf([application({ hire: hire({ acknowledged_at: '2026-09-03T18:06:00.000Z' }) })]);
    renderIntl(<WorkerApplicationsPage />);
  ...
  it('shows no banner on a row that is not hired', () => {
    seed = pageOf([application({ status: 'talking', hire: undefined })]);
    renderIntl(<WorkerApplicationsPage />);
  ...
  it('never opens the celebration modal here -- that belongs to the home page', () => {
    ...
    seed = pageOf([application({ hire: hire({ seen_at: null }) })]);
    renderIntl(<WorkerApplicationsPage />);
