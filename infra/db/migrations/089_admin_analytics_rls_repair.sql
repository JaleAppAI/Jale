...
-- Run manually AFTER 088_admin_analytics.sql, connected as jale_admin (NOT the
-- RDS master user). Forward-only (ADR-005) — 088 is committed and is NOT edited.
--
-- DEFECT REPAIRED
-- 088's five SECURITY DEFINER functions are owned by jale_admin, whose
...
--
-- The failure was silent: no error, just zeros. 088's own DO block did not catch
...
-- role — its only new capability is the five EXECUTE grants 088 already gave it,
-- re-asserted below.
--
...
-- Re-asserted verbatim from 088. CREATE OR REPLACE by the same owner preserves
-- ACLs, so these are no-ops on a database that already carries 088 — they are
...
BEGIN
  ...
  -- Smoke-execute every function. Unlike 088's block, these calls must also
