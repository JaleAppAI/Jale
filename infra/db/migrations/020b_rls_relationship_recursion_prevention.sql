...
-- Run manually AFTER 020_worker_pii_rls_hardening.sql (BEFORE 021)
-- Connect as: jale_admin (NOT the RDS master user)
--
-- OPERATOR RUNBOOK NOTE (why this file exists, out of numeric order):
--
-- Migration 020 adds `users_employer_applicant_read ON users ... TO
-- jale_admin`, whose USING clause joins job_applications -> jobs. But the
-- migration-003 policies on jobs/job_applications (and 007's
-- jobs_worker_read_active) were written WITHOUT a `TO` clause, so PostgreSQL
-- applies them to PUBLIC -- including jale_admin. Because these tables have
-- FORCE ROW LEVEL SECURITY, even the table owner is subject to them. The
-- result is a plan-time policy-expansion cycle: users -> job_applications/
-- jobs -> users -> ... PostgreSQL detects this and raises 42P17 "infinite
-- recursion detected in policy for relation users" the first time ANY
...
-- `jale_admin` -- see infra/lib/stacks/database-stack.ts's
-- `rds.Credentials.fromGeneratedSecret('jale_admin')`) is a member of
-- `rds_superuser`, but role ATTRIBUTES such as BYPASSRLS do NOT inherit
-- through role membership in PostgreSQL -- only privileges/grants do. RDS's
-- `rds_superuser` itself does not carry BYPASSRLS, and membership in it
-- would not grant BYPASSRLS to jale_admin even if it did. jale_admin's own
-- `rolbypassrls` attribute is false, exactly like the plain
-- NOSUPERUSER/NOBYPASSRLS/CREATEROLE role this repo's accepted operational
