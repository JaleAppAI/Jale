/**
 * email-outbox-090.integration.test.ts
 *
 * PostgreSQL-backed suite for migration 090 (email_outbox delivery metadata +
 * public.disable_digest_for_employer).
 *
 * Connection: set JALE_TEST_DATABASE_URL to a local Postgres 16 URL with the
 * full migration chain (001->090) applied, connected as a superuser (e.g.
 ...
 *        npx jest --runInBand test/unit/db/email-outbox-090
 *
...
function maybeDescribe(name: string, fn: () => void): void {
  if (!databaseUrl) {
    ...
    console.warn(
      `[email-outbox-090.integration] SKIPPED: "${name}" -- set JALE_TEST_DATABASE_URL to run `
...
const SUB_BOUNCED = 'outbox-090-employer-bounced';
const SUB_OFF = 'outbox-090-employer-off';
const ALL_SUBS = [SUB_BOUNCED, SUB_OFF];
const EMAIL_LIKE = 'outbox-090-%@example.test';
...
async function insertOutbox(
  ...
  const inserted = await client.query<{ id: string }>(
    `INSERT INTO email_outbox
     ...
     RETURNING id`,
    [
      overrides.recipientEmail ?? 'outbox-090-a@example.test',
...
maybeDescribe('email_outbox delivery metadata + digest feedback definer (migration 090)', () => {
  beforeAll(async () => {
      ...
      for (const sub of ALL_SUBS) {
        const inserted = await client.query<{ id: string }>(
          `INSERT INTO users (cognito_sub, user_type, email, phone, full_name, created_at, updated_at)
           VALUES ($1, 'employer', $2, '+15550000000', 'Outbox 090 Fixture', NOW(), NOW())
           RETURNING id`,
  ...
  /**
   * The reconciliation 090's header promises. 037:32 bakes the attempt bound
   ...
   * (both 5), and 090 deliberately changed NEITHER -- moving either one would
  ...
  it('lets many rows sit at ses_message_id NULL but refuses a duplicate provider id', async () => {
    await withClient(superuserUrl, async (client) => {
      ...
      await insertOutbox(client, { sesMessageId: 'ses-090-unique-1' });
      await expect(insertOutbox(client, { sesMessageId: 'ses-090-unique-1' })).rejects.toMatchObject({
        code: '23505',
      });
      await expect(insertOutbox(client, { sesMessageId: 'ses-090-unique-2' })).resolves.toBeTruthy();
    });
  });
    ...
    const updated = await withClient(adminUrl, async (client) => {
      const result = await client.query(
        `UPDATE email_outbox
          ...
          WHERE id = $1`,
        [outboxId, 'ses-090-sweeper', '{"unsubscribe_url":"https://jaleapp.ai/u"}'],
      );
   ...
   * column now has two writers -- this upsert and 090's bounce definer -- so
