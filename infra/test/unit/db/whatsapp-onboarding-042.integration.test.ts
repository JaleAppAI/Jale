...
maybeDescribe('migration 042 WhatsApp onboarding gate', () => {
  ...
  afterAll(async () => {
    ...
    await setup.query('DELETE FROM worker_identity_challenges WHERE phone_hash = ANY($1::text[])', [[hashA, hashB]]);
  ...
  test('pre-auth functions expose only one exact validated hash', async () => {
    ...
    expect((await asWhatsapp('SELECT id FROM worker_identity_challenges')).rows).toHaveLength(0);
