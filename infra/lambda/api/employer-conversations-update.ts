...
import { closeEmployerConversation, getEmployerConversationDetail } from '../lib/job-messaging';
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  ...
  try {
    ...
    const parsed = JSON.parse(event.body ?? '{}') as { status?: unknown };
    if (parsed.status !== 'closed') {
      return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ error: 'invalid_status' }) };
    }
    ...
    const updated = await closeEmployerConversation(client, conversationId, employerId);
    ...
    console.error('employer-conversations-update error:', errorMessage(err));
