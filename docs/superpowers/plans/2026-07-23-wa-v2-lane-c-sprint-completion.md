...
**D2 — 044 migration is REQUIRED (O1).** `worker_onboarding_state` has a `jale_admin` definer policy (042:247) but `worker_message_intents` has **no jale_admin policy** under FORCE RLS — a sweep-as-`jale_admin` SELECT on intents returns 0 rows silently (false green). 044 adds a least-privilege **`jale_admin` SELECT policy** on `worker_message_intents` (mirrors the onboarding-state definer, SELECT-only — the sweep never writes intents; the drain does, as `jale_whatsapp`). This same policy also serves O3's read-only state display. Optional additive partial index for the eligibility query.
